...
from app.infrastructure.tools.mcp.mcp_servers import search_mcp_client, baidu_mcp_client
...
async def consult_technical_expert_impl(query: str) -> str:
    try:
        logger.info(f"[Route] 转交技术专家: {query[:30]}...")
...
async def query_service_station_and_navigate_impl(query: str) -> str:
    try:
        logger.info(f"[Route] 转交业务专家: {query[:30]}...")
...
async def run_technical_tool():
    ...
    for query in test_cases:
        print(f"\n 查询: {query}")
        print("-" * 0)
        result = await consult_technical_expert_impl(query=query)
        print(f"回答: {result}\n")
...
async def run_service_tool():
    ...
    for query in test_cases:
        print(f"\n查询: {query}")
        print("-" * 80)
        result = await query_service_station_and_navigate_impl(query=query)
        print(f"回答: {result}\n")
